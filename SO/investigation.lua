...
investigation.startWander = function(npcRef)
    ...
    local wanderRange = npcRef.mobile.cell.isOrBehavesAsExterior
        and config.wanderRangeExterior
        or  config.wanderRangeInterior
...
local function returnToOriginalPosition(e)
    ...
    if not npcRefSH:valid() then log:debug("Reference no longer valid in returnToOriginalPosition") return end
...
local function checkDestination(e)
    ...
    if not mob.position then log:debug("Nil position for %s before distance check", npcRef.id) return end
...
investigation.startTravel = function(npcRef, destination)
    ...
    if doChecks(npcRef) then log:debug("NPC is doing other stuff") return end
    ...
    timer.delayOneFrame(function()
        if not npcRefSafe:valid() then log:debug("NPC ref handle got invalidated before travel") return end
